...
function drawEyes(ctx: CanvasRenderingContext2D, keypoints: Face['keypoints']) {
  const leftEyePoints = keypoints.filter((point) =>
    point.name?.includes('leftEye'),
  );
  const rightEyePoints = keypoints.filter((point) =>
    point.name?.includes('rightEye'),
  );
...
function drawEyebrows(
  ...
  const leftEyebrowPoints = keypoints.filter((point) =>
    point.name?.includes('leftEyebrow'),
  );
  const rightEyebrowPoints = keypoints.filter((point) =>
    point.name?.includes('rightEyebrow'),
  );
