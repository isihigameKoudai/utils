...
export type Queries<S extends StateProps> = Record<string, (state: S) => any>;

type Context<S extends StateProps, Q extends Queries<S>> = {
  state: S;
  queries: StoreQueries<Q>;
  dispatch: Dispatch<S>;
};

export type Actions<S extends StateProps, Q extends Queries<S>> = {
...
export type StoreActions<S extends StateProps, Q extends Queries<S>, A extends Actions<S, Q>> = {
  [K in keyof A]: A[K] extends (context: any, ...args: infer P) => any 
    ? (...args: P) => ReturnType<A[K]> 
    : never;
};

export type Store<S extends StateProps, Q extends Queries<S>, A extends Actions<S, Q>> = {
  useStore: () => {
    state: S;
    queries: StoreQueries<Q>;
